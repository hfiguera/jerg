...
-spec load_source(string(), string() | undefined) -> graph:digraph().
load_source(Source, Prefix) ->
...
-spec load_schemas(list(string()), graph:digraph(), string() | undefined) -> ok.
load_schemas([], _, _) ->
    ok;
...
-spec load_single_schema(string(), graph:digraph(), string() | undefined) -> digraph:vertex().
load_single_schema(SchemaFile, SchemaGraph, Prefix) ->
...
-spec get_schema(graph:digraph(), binary()) -> {binary(), jsx:json_term()}.
...
-spec set_schemas_relationships(graph:digraph()) -> ok.
set_schemas_relationships(SchemaGraph) ->
    set_schemas_relationships(
      SchemaGraph,
      get_schema_vertices(SchemaGraph)).

-spec set_schemas_relationships(graph:digraph(), list(jsx:json_term())) -> ok.
...
-spec set_schema_relationships(graph:digraph(), jsx:json_term(), list(jsx:json_term())) -> ok.
...
-spec ensure_no_cyclicity(graph:digraph()) -> ok.
ensure_no_cyclicity(SchemaGraph) ->
...
-spec output_records(graph:digraph(), file:io_device()) -> ok.
...
-spec get_schema_vertices(graph:digraph()) -> list(digraph:vertex()).
get_schema_vertices(SchemaGraph) ->
    get_schema_vertices(SchemaGraph, digraph:no_edges(SchemaGraph)).

-spec get_schema_vertices(graph:digraph(), non_neg_integer()) -> list(digraph:vertex()).
...
-spec output_records(graph:digraph(), list(digraph:vertex()), file:io_device()) -> ok.
output_records(_, [], _) ->
    ok;
...
-spec output_record(graph:digraph(), binary(), jsx:json_term(), file:io_device()) -> ok.
output_record(SchemaGraph, SchemaId, JsonSchema, IoDevice) ->
...
-spec output_object_record(graph:digraph(), binary(), jsx:json_term(), file:io_device()) -> no_return().
output_object_record(SchemaGraph, SchemaId, JsonSchema, IoDevice) ->
...
-spec output_properties(graph:digraph(), jsx:json_term(), file:io_device()) -> no_return().
output_properties(SchemaGraph, JsonSchema, IoDevice) ->
...
-spec output_property(graph:digraph(), list(jsx:json_term()), file:io_device(), integer(), integer()) -> ok.
...
-spec format_default_value(graph:digraph(), jsx:json_term(), integer()) -> binary().
...
-spec collect_properties(graph:digraph(), jsx:json_term()) -> list(jsx:json_term()).
...
-spec find_longest_default_value(graph:digraph(), jsx:json_term()) -> binary().
...
-spec get_property_default_value(graph:digraph(), jsx:json_term()) -> binary().
...
-spec convert_property_default_value(graph:digraph(), jsx:json_term(), term()) -> binary().
convert_property_default_value(SchemaGraph, PropertySchema, DefaultValue) ->
    TypeSpec = get_property_type_spec(SchemaGraph, PropertySchema),
    convert_property_value(DefaultValue, TypeSpec).

-spec convert_property_value(graph:digraph(), binary()) -> binary().
convert_property_value(_, <<"term()">>) ->
    <<>>;
convert_property_value(DefaultValue, _) ->
    format_to_binary("~p", [DefaultValue]).

-spec get_property_type_spec(graph:digraph(), jsx:json_term()) -> binary().
...
-spec get_schema_reference_type_spec(graph:digraph(), binary()) -> binary().
get_schema_reference_type_spec(SchemaGraph, Ref) ->
...
-spec convert_property_type_spec(graph:digraph(), jsx:json_term(), jsx:json_term()) -> binary().
